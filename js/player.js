...
function onMIDIInit( midi ) {
    ...
    midi.inputs.forEach( function ( thisMIDI ) {
        onMIDIConect( thisMIDI );
    } );

    midi.onstatechange = function ( event ) {
        console.log( "MIDIConnectionEvent on port", event.port );
        if ( event.port.type === 'input' &&
            event.port.connection === "open" &&
            event.port.name === "nanoKONTROL2 SLIDER/KNOB" ) {
...
function onMIDIConect( input ) {
    ...
    input.onmidimessage = function ( event ) {
        if ( event.data[ 0 ] === 176 && event.data[ 1 ] > 15 && event.data[ 1 ] < 24 ) {
            updateParam( event.data[ 1 ] - 16, event.data[ 2 ] );
        }
    };
}
...
function updateParam( ch, value ) {
    var modelParams = window.sndModel.listParams();
    var chosenParam = modelParams[ ch ];
    if ( chosenParam ) {
        if ( typeof chosenParam.value === 'number' ) {
            var scaledVal = ( ( value / 127 ) * ( chosenParam.maxValue - chosenParam.minValue ) ) + chosenParam.minValue;
            $( "#" + chosenParam.name + "val" )
                .val( scaledVal );
            // Update Slider thumb with text input
            $( "#" + chosenParam.name )
                .tickslider( "value", scaledVal );
        } else {
            $( "." + chosenParam.name )
                .bootstrapSwitch( "setState", value > 64, true );
...
function swapModels( mc ) {
    ...
    // Load model class
    window.sndModel = new Model( context, files, null, onLoad, onAudioStart, onAudioEnd );
    ...
    function onLoad( status ) {
        ...
        generateInterface( window.sndModel );
        // Generate sliders again everytime a new file is loaded.
        generateParam( window.sndModel );
    }
    ...
    function handleFileSelect( evt ) {
        ...
        if ( window.sndModel.isPlaying ) {
            window.sndModel.stop();

        }
        // Remove sliders
        $playerParams.empty();
        window.sndModel.setSources( localSources, null, onLoad );
    }
